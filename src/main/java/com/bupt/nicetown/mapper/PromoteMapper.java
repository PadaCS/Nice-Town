...
public interface PromoteMapper {
    @Insert("INSERT INTO Promote (PromotterID, TownID, Theme, PromoteType, Description, images, videos, status) " +
            "VALUES (#{promotterID}, #{townID}, #{theme}, #{promoteType}, #{description}, #{images}, #{videos}, 0)")
    ...
    @Update("update promote set townID=#{townID}, PromoteType=#{promoteType}, theme=#{theme}, description=#{description}, images=#{images}, videos=#{videos} where PromotterID=#{promotterID} and promoteID=#{promoteID}")
